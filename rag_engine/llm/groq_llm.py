"""Groq LLM backend (OpenAI-compatible API)."""
...
class GroqLLM(BaseLLM):
    """Chat completion via Groq's blindingly fast API (OpenAI-compatible)."""
    ...
    def __init__(
        ...
        logger.info(
            "GroqLLM initialized | model=%s | base_url=https://api.groq.com/openai/v1",
            self._model,
        )
    ...
    def complete_with_messages(self, messages: list[dict]) -> str:
        ...
        logger.info(
            "GroqLLM response | model=%s | tokens_used=%s | chars=%d",
            self._model,
