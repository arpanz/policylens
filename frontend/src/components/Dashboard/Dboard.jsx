...
export default function Dboard({ file, isDark: _initDark, userName = 'My Account', initialPolicyId = null }) {
  ...
  useEffect(() => {
    ...
    if (initialPolicyId && !hasAutoProcessed.current) {
      ...
      fetchSummary(initialPolicyId);
    } else if (file && !hasAutoProcessed.current) {
      hasAutoProcessed.current = true;
      processFile(file);
    }
  ...
  const Badge = ({ label, variant='acc' }) => {
    const map = { acc:[T.badgeBg,T.badgeClr], green:[T.greenBg,T.greenClr], red:[T.redBg,T.redClr], amber:[T.amberBg,T.amberClr] };
    const [bg, clr] = map[variant] || map.acc;
    return <span style={{ display:'inline-flex', alignItems:'center', padding:'2px 10px', borderRadius:20, ...mono, fontSize:10, fontWeight:600, background:bg, color:clr }}>{label}</span>;
  };

  const InfoCard = ({ icon:Icon, label, value, variant='acc' }) => {
    const map = { acc:[T.accBg,T.acc], green:[T.greenBg,T.greenClr], amber:[T.amberBg,T.amberClr] };
      ...
      <div style={{ borderRadius:16, padding:'18px 20px', background:T.cardBg, border:`1px solid ${T.cardBorder}`, boxShadow:T.cardShadow }}>
        <div style={{ display:'flex', alignItems:'flex-start', gap:12 }}>
          ...
          <div style={{ minWidth:0 }}>
            ...
            <p style={{ ...f, fontSize:13, lineHeight:1.65, color: value ? T.t2 : T.t3, margin:0, fontStyle: value ? 'normal' : 'italic' }}>{value || 'Not applicable'}</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="fade-in" style={{ display:'flex', flexDirection:'column', gap:20 }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20 }}>
        <div style={{ borderRadius:20, padding:'22px 24px', background:T.cardBg, border:`1px solid ${T.cardBorder}`, boxShadow:T.cardShadow }}>
          <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:16 }}>
            <div style={{ width:30, height:30, borderRadius:8, display:'flex', alignItems:'center', justifyContent:'center', background:T.greenBg }}><CheckCircle2 size={15} style={{ color:T.greenClr }} /></div>
        ...
        <div style={{ borderRadius:20, padding:'22px 24px', background:T.cardBg, border:`1px solid ${T.cardBorder}`, boxShadow:T.cardShadow }}>
          <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:16 }}>
            <div style={{ width:30, height:30, borderRadius:8, display:'flex', alignItems:'center', justifyContent:'center', background:T.redBg }}><AlertTriangle size={15} style={{ color:T.redClr }} /></div>
      ...
      <div style={{ borderRadius:20, padding:'22px 24px', background:T.cardBg, border:`1px solid ${T.cardBorder}`, boxShadow:T.cardShadow }}>
        <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:18 }}>
          <div style={{ width:30, height:30, borderRadius:8, display:'flex', alignItems:'center', justifyContent:'center', background:T.amberBg }}><Info size={15} style={{ color:T.amberClr }} /></div>
    ...
    <div style={{ display:'flex', height:'100vh', overflow:'hidden', background:T.pageBg, color:T.t1, ...f, transition:'background .4s, color .4s' }}>
      ...
      <aside style={{ flexShrink:0, display:'flex', flexDirection:'column', width: sidebarOpen ? 240 : 66, background:T.sidebarBg, borderRight:`1px solid ${T.headerBorder}`, transition:'width .3s cubic-bezier(0.16,1,0.3,1)', overflow:'hidden' }}>
        ...
        <div style={{ padding:'12px 8px', borderTop:`1px solid ${T.headerBorder}` }}>
          ...
          >
            {dark ? <Sun size={15} style={{ color:'#22d3ee', flexShrink:0 }} /> : <Moon size={15} style={{ color:T.acc, flexShrink:0 }} />}
          ...
          <div className="cb-scrollbar" style={{ flex:1, overflowY:'auto', padding:'20px 16px', display:'flex', flexDirection:'column', gap:16, background:T.msgAreaBg }}>
            ...
            {isTyping && (
              <div className="cb-msg" style={{ display:'flex', gap:10, alignItems:'center' }}>
                <div style={{ width:32, height:32, borderRadius:10, flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center', background:T.msgAiBg, border:`1px solid ${T.cardBorder}` }}>
                  <Loader2 size={14} className="iris-spinner" style={{ color:T.acc }} />
                </div>
